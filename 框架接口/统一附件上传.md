...
1.**功能介绍**：提供附件上传、下载、生成缩略图、附件上传记录查询功能，支持上传文件至应用服务器以及指定FTP服务器。

2.**参数配置**：配置文件 `attachment.upload.config.json` 对不同的`文件系统`以及`FTP服务器`进行附件上传相关配置，具体参数说明如下：
```javascript
{
  "components": [
    {
...
| Path           | string                  | 附件路径（FTP路径或绝对路径），`生成规则：（components.rootpath 或 ftpservers.ip）/ModuleName/CaseType`   |
