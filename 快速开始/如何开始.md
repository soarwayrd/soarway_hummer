...
2. 新建项目解决方案
>1. 打开Visual Studio 2017点击文件 -> 选择新建 -> 选择项目 ->选择SoarwayHummer开发框架。输入你的项目名称、输入框架所在文件夹名称

>2. **默认是IIS Express启动，修改为控制台应用程序启动选择项目的*.API** 

>3. 添加框架的XML文件*Soarway.Hummer.Core.API.xml*与项目*Soarway.Hummer.Demo.API.xml*文件。项目的XML生成方式见下图。选择*.API的项目右键->属性->生成 。见下图

![GitHub](../accets/projectxmlsetting.png)

>4. 修改数据库连接串，数据库连接串配置在*Config/Setting/common.core.config.json*。*dbCore*修改成自己的SQL Server数据库地址、账号、密码。
...
7. 新建服务接口
>* 找到*Soarway.Hummer.Demo.IService*项目，添加*IStudentService.cs*接口文件, 并实现如下代码。

>* 找到*Soarway.Hummer.Demo.DTO*项目，新建Student文件夹添加StudentCreateDTO.cs。使用DTO这种方式好处是每个业务有自己单独的DTO的方法互不影响各自的业务。

>* 添加引用Soarway.Hummer.Core.Infrastructure;
...
```
8. 新建服务实现
>* 找到*Soarway.Hummer.Demo.Service*项目，添加*StudentService.cs*文件, 并实现如下代码和引用。

>* 添加AutoMapper、Soarway.Hummer.Core.Infrastructure、Soarway.Hummer.Core.DTO、Soarway.Hummer.Demo.DbSet、Soarway.Hummer.Demo.DTO、Soarway.Hummer.Demo.IRepository、Soarway.Hummer.Demo.IService等引用

>* 添加*IStudentRepository*仓储注入
...
>1. <a href="http://www.ruanyifeng.com/blog/2018/10/restful-api-best-practices.html" target="_blank">RESTful API 最佳实践</a>
>2. <a href="http://www.ruanyifeng.com/blog/2014/05/restful_api.html" target="_blank">RESTful API 设计指南</a>
>3. <a href="https://www.cnblogs.com/cgzl/p/9010978.html" target="_blank">用ASP.NET Core 2.0 建立规范的 REST API -- 预备知识</a>
>4. <a href="https://www.cnblogs.com/cgzl/p/9019314.html" target="_blank">用ASP.NET Core 2.0 建立规范的 REST API -- 预备知识 (2) + 准备项目</a>
>5. <a href="https://www.cnblogs.com/cgzl/p/9047626.html" target="_blank">用ASP.NET Core 2.0 建立规范的 REST API -- GET 和 POST</a>
>6. <a href="https://www.cnblogs.com/cgzl/p/9080960.html" target="_blank">用ASP.NET Core 2.0 建立规范的 REST API -- DELETE, UPDATE, PATCH 和 Log</a>
>7. <a href="https://www.cnblogs.com/cgzl/p/9117448.html" target="_blank">用ASP.NET Core 2.1 建立规范的 REST API -- 翻页/排序/过滤等</a>

1.  Http Status Code 简介
* 1xx, 用于通知报告.
...
>1. 找到*Soarway.Hummer.Demo.API*项目，点击Controllers文件夹，添加->控制器->选择 *API 控制器 - 空* 。将该*Controller*继承至*BaseController*
>2. 注入 *IStudentService*服务。
>3. 添加Soarway.Hummer.Core.DTO、Soarway.Hummer.Core.Infrastructure、Soarway.Hummer.Demo.DTO、Soarway.Hummer.Demo.IService、System.ComponentModel等引用。
>4. 找到*Soarway.Hummer.Demo.Infrastructure*项目，点击Mapper文件夹，找到*BizMapper.cs* 添加AutoMapper关系,并实现如下代码。
